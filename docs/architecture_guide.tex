...
\subsubsection{Step 3: BFS Calls the Callback Per Node}

Progress is reported \textbf{once per node} rather than once per batch. This keeps the UI updating smoothly even when BFS is processing a large batch of pages. The bulk Wikipedia fetch is unaffected — the only overhead is one extra Redis \texttt{SET} (via \texttt{update\_state}) per node, which costs $\approx$0.1--0.3\,ms on a co-located Redis instance.

\begin{lstlisting}[caption=pathfinding.py -- Per-Node Progress Reporting]
def _perform_bfs_search(self, start_page, end_page, ...):
    nodes_explored = 0

    while self.queue_service.length(queue_key) > 0:
        # Pop a batch; fetch all their links in one bulk API call
        batch_items = self.queue_service.pop_batch(queue_key, self.batch_size)

        # Fire callback once per node for smooth client-side progress.
        # bulk fetch below is unchanged -- per-node callbacks add only cheap
        # Redis SET ops, not extra Wikipedia API calls.
