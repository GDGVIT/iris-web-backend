...
\noindent\textbf{Queue size is intentionally stale within a batch.}  \texttt{queue\_size} is read by calling \texttt{self.queue\_service.length(queue\_key)} inside the callback, which fires \emph{during} the Wikipedia fetch, before the newly discovered links from this batch have been enqueued.  The value therefore reflects the queue after the \texttt{pop\_batch} removed the current batch, but before the link-processing loop pushes any new nodes.  This is a known and acceptable trade-off: \texttt{queue\_size} is a rough UI hint, not a precise count.  \texttt{nodes\_explored} and \texttt{current\_depth} are always accurate.
