...
\begin{itemize}[leftmargin=*, nosep]
    ...
    \item \textbf{Throughput:} Popping 50 nodes and issuing one bulk API call replaces 50 sequential API round-trips per BFS level. Wikipedia's API accepts up to 50 titles per request.
...
\begin{decisionbox}{Two-Level Batching: BFS Queue + Wikipedia API}
\textbf{Problem:} Fetching links one page at a time makes both the BFS loop and the Wikipedia API inefficient.

\textbf{Decision:} Two coordinated batch sizes working together:
\begin{enumerate}[leftmargin=*, nosep]
    \item \textbf{BFS layer} pops \texttt{BFS\_BATCH\_SIZE} nodes per iteration and passes them all to \texttt{get\_links\_bulk}.
    \item \textbf{WikipediaClient} groups those titles into sub-batches of up to \texttt{WIKIPEDIA\_BATCH\_SIZE} (≤50, the API hard limit) and dispatches them in parallel via \texttt{ThreadPoolExecutor}.
\end{enumerate}

\textbf{Engineering Justification:}
\begin{itemize}[leftmargin=*, nosep]
    \item \textbf{I/O Bound:} Wikipedia API calls are network-bound; threads maximise throughput.
    \item \textbf{Configurable:} \texttt{WIKIPEDIA\_MAX\_WORKERS} controls parallelism (default: 10). \texttt{WIKIPEDIA\_API\_TIMEOUT} controls per-request timeout (default: 15s).
...
def _perform_bfs_search(self, start_page, end_page, ...):
    ...
    while self.queue_service.length(queue_key) > 0:
        ...
        links_bulk = self.wikipedia_client.get_links_bulk(
            [item["page"] for item in batch_items]
        )
